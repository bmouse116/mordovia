        ...
        <div class="point-info" v-if="tenantData">
            ...
            <div class="point-body">
                ...
                <div class="point-description">
                    ...
                    <div class="point-trainers" v-if="filteredTrainers.length !== 0">
                ...
                <div class="point-controls">
                    <button @click="showOnMap">
                        <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M32.5 4C21.1962 4 12 13.098 12 24.281C12 38.1594 30.3455 58.5337 31.1266 59.3942C31.8602 60.2026 33.1411 60.2012 33.8734 59.3942C34.6545 58.5337 53 38.1594 53 24.281C52.9998 13.098 43.8036 4 32.5 4ZM32.5 34.4849C26.8128 34.4849 22.186 29.9075 22.186 24.281C22.186 18.6545 26.8129 14.0772 32.5 14.0772C38.1871 14.0772 42.8138 18.6546 42.8138 24.2811C42.8138 29.9076 38.1871 34.4849 32.5 34.4849Z"
                                fill="#415AFF" />
                        </svg>
                        Показать на карте
                    </button>
...
import type { Floor, Service, Tenants } from '../types/types';
...
const showOnMap = () => {
    ...
    if (destinationPointId) {
        router.push({
            path: '/home', // или name: 'home'
            // Используем другой query-параметр, чтобы отличать от построения маршрута
            query: {
                focusOnId: destinationPointId 
            }
        });
    }
};
...
const selectImage = (index: number) => {
    ...
    nextTick(() => {
        const activeThumbnail = thumbnailRefs.value[index];
        if (activeThumbnail) {
...
const selectNextImage = () => {
    if (tenantData.value?.images && tenantData.value.images.length > 0) {
        ...
        nextTick(() => {
            const activeThumbnail = thumbnailRefs.value[nextIndex];
